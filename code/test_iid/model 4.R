########################################
# Correlated Fixed Effects for Model 4 #
########################################

# model4 of DGP2 in the file "DGPs":
...
for(c in 1:ncase){
  ...
  for (i in 1:nsim){
    ...
    Y_list<-dgp$Y_list
    df<-dgp$df
    beta_hat_0<-plm(y_it ~ x_it_1+x_it_2+x_i+w_t, data=df, model="pooling")$coefficients %>% as.matrix()
    beta_hat_0[c(3,1,2)] <- beta_hat_0[c(1,2,3)]
...
write.csv(rst, file = paste0( "model 4_", nsim, ".csv"), row.names = F)

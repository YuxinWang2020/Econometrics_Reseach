########################################
# Correlated Fixed Effects for Model 3 #
########################################

# model3 of DGP2 in the file "DGPs":
# y_it = mu + beta1*x_it_1 + beta2*x_it_2 + Lambda_i*Factor_t + eps    
# p=3, X=(x1,x2,1)
...
DGP <- function(T_, N, beta_true){
  ...
  Factor <- rbind(matrix(arima.sim(model=list(ar=0.7), n=T_), nrow=1, ncol=T_),
                  matrix(arima.sim(model=list(ar=0.7), n=T_), nrow=1, ncol=T_))
  ...
  Y <- beta_true[1]*X_1 + beta_true[2]*X_2 + mu*X_3 + Lambda_Factor + Eps
...
rst<-data.frame(matrix(NA, nrow=ncase*2,ncol=9))
...
write.csv(rst, file = paste0( "model 3_", nsim, ".csv"), row.names = F)
